...
exports.searchTeacher = async(req, res) => {
    try {
        ...
        const teacherData = teacher.map((professor) => professor.get({ plain: true }));
...
exports.addDisciplineToTeacher = async (req, res) => {
  try {
    ...
    if (!id_professor || !id_disciplina) {
      return res.status(400).json({ error: "professorId e disciplinaId são obrigatórios." });
    }
    ...
    if (isDiscipline) {
      return res.status(400).json({ error: "Esta disciplina já está vinculada a este professor." });
    }
    ...
    return res.status(500).json({ error: "Erro ao adicionar disciplina ao professor" });
  }
};
